            ...
            for i in range(0,len(vector_barrier),4):
                ...
                if next_point_vec >= temp[0][0] and next_point_vec <= temp[-1][0]:
                    if euler[batch_num] > temp_euler[0]:
                        ...
                        if  vec_dis  < self.vec_euler([euler[batch_num],barrier_z_high]):
                            rewards[batch_num] += vec_dis - euler[batch_num]
                        else:
                            rewards[batch_num] += self.vec_euler([euler[batch_num],barrier_z_high]) - euler[batch_num]
                            del self.barrier_path[-2:]
                    else:
                        pass
                else:
                    pass
